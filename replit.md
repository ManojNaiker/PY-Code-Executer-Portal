...
- Build subsystem is **conditional**: GUI scripts (tkinter, PyQt, wx, kivy, pygame, etc.) get linked with `-H windowsgui` (no console window) and use `pythonw.exe`; CLI scripts get the default Windows **console subsystem** so `print()` output is visible. CLI EXEs also pause for Enter at exit so double-click users can read the output before the window closes.
